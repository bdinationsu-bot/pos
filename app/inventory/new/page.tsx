...
export default function NewDevicePage() {
  ...
  const batteryColor = f.battery_health >= 90 ? 'text-green-600'
    ...
    : f.battery_health > 0 ? 'text-red-600'
    : 'text-gray-400'

  const gradeColor: any = {
    A: 'bg-green-100 text-green-800 border-green-500',
    B: 'bg-blue-100 text-blue-800 border-blue-500',
    C: 'bg-yellow-100 text-yellow-800 border-yellow-500',
    D: 'bg-red-100 text-red-800 border-red-500'
  }
  ...
  return (
    <div className="p-6 max-w-4xl">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2">
            ...
            <div className="text-xs text-gray-500 mt-1">
              {f.imei.length}/15 လုံး {f.imei.length === 15 && '✅'}
            </div>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          <div>
            ...
            <input
              ...
              onChange={e => setF({ ...f, battery_health: +e.target.value || 0 })}
              className={`border p-2 rounded w-full text-2xl font-bold text-center ${batteryColor}`}
              placeholder="0"
              min="0"
              max="100"
            />
            <div className={`text-center text-xs mt-1 font-medium ${batteryColor}`}>
              {f.battery_health >= 90 ? '✅ ကောင်း' :
               f.battery_health >= 80 ? '⚠️ သင့်တင့်' :
               f.battery_health > 0 ? '❌ အားနည်း' : ''}
            </div>
          </div>
          ...
          <div>
            ...
            <input
              type="number"
              value={f.cycle_count || ''}
              onChange={e => setF({ ...f, cycle_count: +e.target.value || 0 })}
          ...
          <div>
            <label className="block text-sm mb-1 font-medium">Grade</label>
              ...
              {['A', 'B', 'C', 'D'].map(g => (
                <button
                  ...
                  className={`py-2 rounded border-2 font-bold transition ${
                    f.grade === g ? gradeColor[g] : 'bg-white border-gray-300 text-gray-400'
                  }`}
                >
                  {g}
                </button>
      ...
      <div className="bg-white rounded shadow p-4 mb-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select
              value={f.supplier_id ?? ''}
              onChange={e => setF({ ...f, supplier_id: e.target.value ? +e.target.value : null })}
              className="border p-2 rounded w-full"
            >
            ...
            <div className="text-xs text-gray-500 mt-1">
              Supplier မရှိသေးရင် <a href="/suppliers" className="text-green-700 underline">ဒီမှာ ထည့်ပါ</a>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded shadow p-4 mb-4">
        ...
        <input
          value={f.parts_changed}
          onChange={e => setF({ ...f, parts_changed: e.target.value })}
          placeholder="ဥပမာ — Screen အသစ်, Battery အသစ်"
          className="border p-2 rounded w-full mb-3"
        />
        <textarea
          value={f.condition_note}
          onChange={e => setF({ ...f, condition_note: e.target.value })}
          placeholder="အခြား မှတ်ချက်..."
          className="border p-2 rounded w-full"
          rows={2}
        />
      </div>
      ...
      <div className="bg-white rounded shadow p-4 mb-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div>
            <label className="block text-sm mb-1 font-medium">ဝယ်ဈေး (Cost)</label>
            <input
              type="number"
              value={f.cost_price || ''}
              onChange={e => setF({ ...f, cost_price: +e.target.value || 0 })}
              className="border p-2 rounded w-full text-lg font-bold"
              placeholder="0"
            />
          </div>
          <div>
            <label className="block text-sm mb-1 font-medium">ရောင်းဈေး (Sale)</label>
            <input
              type="number"
              value={f.sale_price || ''}
              onChange={e => setF({ ...f, sale_price: +e.target.value || 0 })}
          ...
          <div>
            ...
            <input
              ...
              onChange={e => setF({ ...f, warranty_days: +e.target.value || 0 })}
              className="border p-2 rounded w-full text-lg font-bold"
              placeholder="0"
            />
          </div>
          ...
          <div className="mt-3 bg-green-50 border border-green-300 p-3 rounded text-sm">
            ...
            <div className="flex justify-between mt-1">
              ...
              <strong className="text-green-700">
                {f.cost_price > 0 ? (((f.sale_price - f.cost_price) / f.cost_price) * 100).toFixed(1) : 0}%
              </strong>
            </div>
          </div>
        )}
      ...
      <div className="flex gap-2">
        <button
          onClick={save}
          disabled={saving || f.icloud_locked}
        ...
        <button onClick={() => router.push('/inventory')} className="bg-gray-200 px-8 py-3 rounded font-medium">
          ပယ်ဖျက်
        </button>
      </div>
    </div>
