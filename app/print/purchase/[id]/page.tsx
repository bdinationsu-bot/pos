...
export default function PurchasePrint() {
  ...
  return (
    <div style={{ background: '#fff', minHeight: '100vh', padding: '10mm', fontFamily: '-apple-system, Padauk, Arial, sans-serif', color: '#111' }}>
      <style>{'@media print { @page { size: A4; margin: 10mm; } body { margin: 0; background: #fff; } .no-print { display: none !important; } } .no-print { position: fixed; top: 10px; right: 10px; z-index: 1000; background: ' + green + '; color: white; border: none; padding: 12px 20px; border-radius: 8px; cursor: pointer; font-weight: bold; }'}</style>
      ...
      <div style={{ background: green, color: 'white', padding: '20px 25px', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 25 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 15 }}>
          {shop.logo_url ? (
            <img src={shop.logo_url} style={{ width: 70, height: 70, borderRadius: 10, objectFit: 'cover', background: 'white', padding: 4 }} />
          ) : (
          ...
          <div>
            ...
            {shop.shop_phone && <div style={{ fontSize: 12, opacity: 0.95 }}>Phone: {shop.shop_phone}</div>}
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        <div style={{ background: greenLight, borderLeft: '4px solid ' + green, padding: '12px 16px', borderRadius: 6 }}>
          ...
          {supplier?.phone && <div style={{ fontSize: 12, color: '#555', marginTop: 2 }}>Phone: {supplier.phone}</div>}
        </div>
        <div style={{ background: greenLight, borderLeft: '4px solid ' + green, padding: '12px 16px', borderRadius: 6 }}>
          ...
          <table style={{ fontSize: 12, width: '100%' }}>
            <tbody>
              ...
              <tr><td style={{ color: '#666' }}>Type:</td><td style={{ fontWeight: 'bold', textAlign: 'right', textTransform: 'capitalize' }}>{purchase.purchase_type}</td></tr>
            </tbody>
          </table>
        </div>
      </div>
        ...
        <tbody>
          {items.map((it, i) => (
            <tr key={it.id} style={{ borderBottom: '1px solid #e5e7eb', background: i % 2 === 0 ? '#fff' : greenLight }}>
              ...
              <td style={{ padding: '10px 12px', fontSize: 12, textAlign: 'right' }}>{Number(it.unit_cost).toLocaleString()}</td>
              <td style={{ padding: '10px 12px', fontSize: 12, textAlign: 'right', fontWeight: 'bold' }}>{Number(it.total_cost).toLocaleString()}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        <div style={{ background: greenLight, border: '1px solid ' + green, borderRadius: 6, padding: '12px 16px' }}>
          ...
          <div style={{ fontSize: 13, fontWeight: 'bold', color: greenDark, textTransform: 'capitalize' }}>{purchase.payment_method || '-'}</div>
          {purchase.payment_ref && <div style={{ fontSize: 11, color: '#666' }}>Ref: {purchase.payment_ref}</div>}
        </div>
        <div>
          <table style={{ width: '100%', fontSize: 13 }}>
            <tbody>
              ...
              <tr><td style={{ padding: '6px 0', color: '#666' }}>Paid</td><td style={{ padding: '6px 0', textAlign: 'right', color: green }}>{Number(purchase.paid).toLocaleString()} Ks</td></tr>
              <tr><td style={{ padding: '6px 0', fontWeight: 'bold' }}>Balance</td><td style={{ padding: '6px 0', textAlign: 'right', fontWeight: 'bold', color: Number(purchase.balance) > 0 ? '#dc2626' : '#16a34a' }}>{Number(purchase.balance).toLocaleString()} Ks</td></tr>
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div style={{ textAlign: 'center', marginTop: 30, paddingTop: 15, borderTop: '1px solid #e5e7eb' }}>
        <div style={{ fontSize: 13, color: greenDark, fontWeight: 'bold' }}>{shop.footer_text || 'Thank You'}</div>
      </div>
    </div>
  )
}
