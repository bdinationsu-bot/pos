...
export default function InvoicePrint() {
  ...
  return (
    <div style={{ background: '#fff', minHeight: '100vh', padding: '10mm', fontFamily: '-apple-system, "Padauk", "Myanmar Text", Arial, sans-serif', color: '#111' }}>
      ...
      {/* HEADER — Green bar with logo + shop info */}
      <div style={{
        background: green,
        color: 'white',
        padding: '20px 25px',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 15 }}>
          {shop.logo_url ? (
            ...
            <div style={{
              width: 70, height: 70, borderRadius: 10, background: 'white',
              color: green, display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 32, fontWeight: 'bold'
            }}>P</div>
          )}
          <div>
            <div style={{ fontSize: 24, fontWeight: 'bold', letterSpacing: 0.5 }}>
              {shop.shop_name || 'POS'}
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 11, color: greenDark, fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>
            ဖောက်သည် / Bill To
          </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 11, color: greenDark, fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>
            ငွေတောင်းခံလွှာ အချက်အလက်
          </div>
          <table style={{ fontSize: 12, width: '100%' }}>
            <tbody>
              <tr>
                <td style={{ color: '#666', paddingBottom: 3 }}>Invoice No:</td>
                <td style={{ fontWeight: 'bold', textAlign: 'right' }}>{sale.invoice_no}</td>
              </tr>
              <tr>
                <td style={{ color: '#666', paddingBottom: 3 }}>Date:</td>
                <td style={{ fontWeight: 'bold', textAlign: 'right' }}>
                  {new Date(sale.created_at).toLocaleDateString()}
                </td>
              </tr>
              <tr>
                <td style={{ color: '#666', paddingBottom: 3 }}>Time:</td>
                <td style={{ fontWeight: 'bold', textAlign: 'right' }}>
              ...
              {sale.staff?.name && (
                <tr>
                  <td style={{ color: '#666' }}>Staff:</td>
                  <td style={{ fontWeight: 'bold', textAlign: 'right' }}>{sale.staff.name}</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 20 }}>
        <thead>
          <tr style={{ background: green, color: 'white' }}>
            ...
            <th style={{ padding: '10px 12px', textAlign: 'left', fontSize: 12, fontWeight: 'bold' }}>IMEI / Serial</th>
            <th style={{ padding: '10px 12px', textAlign: 'center', fontSize: 12, fontWeight: 'bold' }}>Qty</th>
            <th style={{ padding: '10px 12px', textAlign: 'right', fontSize: 12, fontWeight: 'bold' }}>Unit Price</th>
        ...
        <tbody>
          {items.map((it, i) => (
            <tr key={it.id} style={{ borderBottom: '1px solid #e5e7eb', background: i % 2 === 0 ? '#fff' : greenLight }}>
              <td style={{ padding: '10px 12px', fontSize: 12 }}>{i + 1}</td>
              <td style={{ padding: '10px 12px', fontSize: 12, fontWeight: 500 }}>{it.name}</td>
              <td style={{ padding: '10px 12px', fontSize: 11, fontFamily: 'monospace', color: '#555' }}>
                {it.imei || '-'}
              </td>
              ...
              <td style={{ padding: '10px 12px', fontSize: 12, textAlign: 'right', fontWeight: 'bold' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 25 }}>
        ...
        <div>
          <div style={{
            ...
            <div style={{ fontSize: 11, color: greenDark, fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>
            ...
            {payments.length === 0 ? (
              ...
              payments.map((p, i) => (
                <div key={i} style={{ fontSize: 13, marginBottom: 4 }}>
                  ...
                  {p.ref_no && (
                    <div style={{ fontSize: 11, color: '#666' }}>Ref: {p.ref_no}</div>
                  )}
            ...
            <tbody>
              <tr>
                ...
                <td style={{ padding: '6px 0', textAlign: 'right', fontWeight: 500 }}>
                  {Number(sale.subtotal).toLocaleString()} Ks
                </td>
              </tr>
              {Number(sale.discount) > 0 && (
                <tr>
                  <td style={{ padding: '6px 0', color: '#666' }}>Discount</td>
                  <td style={{ padding: '6px 0', textAlign: 'right', color: '#ea580c' }}>
              ...
              {Number(sale.tradein_amount) > 0 && (
                <tr>
                  <td style={{ padding: '6px 0', color: '#666' }}>Trade-in</td>
                  <td style={{ padding: '6px 0', textAlign: 'right', color: '#2563eb' }}>
                    -{Number(sale.tradein_amount).toLocaleString()} Ks
                  </td>
                </tr>
              )}
              <tr style={{ borderTop: `2px solid ${green}` }}>
                <td style={{ padding: '12px 0 6px', fontSize: 16, fontWeight: 'bold', color: greenDark }}>
                  TOTAL
                </td>
                <td style={{ padding: '12px 0 6px', textAlign: 'right', fontSize: 18, fontWeight: 'bold', color: greenDark }}>
      ...
      {items.some(it => it.item_type === 'device') && (
        <div style={{
          ...
          <div style={{ fontWeight: 'bold', color: greenDark, marginBottom: 4 }}>
            📋 အာမခံ အချက်အလက် / Warranty Terms
          </div>
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ borderTop: '1px solid #999', width: '70%', paddingTop: 4 }}>
            Customer Signature
          </div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ marginBottom: 40, fontSize: 12 }}>ရောင်းချသူ လက်မှတ်:</div>
          <div style={{ borderTop: '1px solid #999', width: '70%', marginLeft: 'auto', paddingTop: 4 }}>
