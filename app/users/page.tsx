...
export default function UsersPage() {
  ...
  async function deleteUser(id: string, email: string) {
    ...
    alert('⚠️ Profile ဖျက်ပြီးပါပြီ\n\nAuth user ကို Supabase Dashboard → Authentication → Users မှာ ဖျက်ရမယ်')
    load()
  }
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            မင်း Role: <strong className="text-green-700 capitalize">{myRole || '...'}</strong>
          </p>
        </div>
        {myRole === 'owner' && (
      ...
      {showInvite && myRole === 'owner' && (
        <div className="bg-white rounded shadow p-4 mb-4">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowInvite(false)} className="bg-gray-200 px-6 py-2 rounded font-medium">
              ပယ်ဖျက်
            </button>
          </div>
        </div>
      ...
      <div className="bg-white rounded shadow overflow-hidden">
        {loading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {list.length === 0 && (
                <tr><td colSpan={6} className="p-8 text-center text-gray-400">User မရှိပါ</td></tr>
              )}
              {list.map(u => (
      ...
      <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded text-sm">
        <strong className="text-blue-800">📌 Role အဓိပ္ပာယ်:</strong>
        <ul className="ml-4 mt-2 space-y-1 text-gray-700">
          <li><strong>Owner</strong> — အားလုံး + Users + Settings</li>
          <li><strong>Manager</strong> — POS, Inventory, Purchases, Accounting, Staff</li>
          <li><strong>Cashier</strong> — POS, Inventory, Trade-in</li>
          <li><strong>Technician</strong> — Repair Desk</li>
          <li><strong>Viewer</strong> — ကြည့်ရုံပဲ</li>
        </ul>
      </div>

      <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded text-sm">
        <strong className="text-yellow-800">⚠️ အရေးကြီး:</strong>
        <div className="mt-2 text-gray-700">
          User အသစ် ဆောက်တဲ့အခါ Email ကို <strong>Auto Confirm</strong> ဖြစ်ဖို့ လိုတယ်။ 
          Supabase Dashboard → <strong>Authentication → Providers → Email</strong> မှာ 
          <strong>"Confirm email"</strong> ကို <strong>OFF</strong> ထားပါ။
          မဟုတ်ရင် user က email verify မလုပ်နိုင်ရင် login ဝင်လို့ မရဘူး။
        </div>
      </div>
    </div>
  )
